...
class ModuleDefinitions
{
	...
	public static findByType(type:string):any
	{
		for(var i = 0; i < ModuleDefinitions._MODULES.length; i++)
		{
			if(ModuleDefinitions._MODULES[i].type === type) return ModuleDefinitions._MODULES[i];
		}

		return null;
	}

	public static findByCategory(category):any
	{
		var results = [];

		for(var i = 0; i < ModuleDefinitions._MODULES.length; i++)
		{
			if(ModuleDefinitions._MODULES[i].category === category) results.push(ModuleDefinitions._MODULES[i]);
		}
